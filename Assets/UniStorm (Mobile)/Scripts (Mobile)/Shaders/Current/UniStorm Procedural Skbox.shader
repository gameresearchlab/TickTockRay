...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		half4 frag (v2f IN) : SV_Target
		{
			...
			if(IN.rayDir.y < 0.02)
			{
				...
				//half mie2 = calcMoonSpot(_WorldSpaceLightPos0.xyz, ray);
				
				col = getRayleighPhase(eyeCos2) * IN.cIn.xyz;
				//col2 = getRayleighPhase(-eyeCos2) * IN.cIn.xyz;
				...
				if(IN.rayDir.y < 0.0)
				{
					...
					//What draws and calculates light!
					//col += mie2;
				}
				else
				{
					...
					//col2 = lerp(col, groundColor, IN.rayDir.y / 0.02);
				}
			}
			else
			{
				col = IN.cIn.xyz + COLOR_2_LINEAR(_GroundColor) * IN.cOut;
				//col2 = IN.cIn.xyz + COLOR_2_LINEAR(_GroundColor) * IN.cOut;
			}
