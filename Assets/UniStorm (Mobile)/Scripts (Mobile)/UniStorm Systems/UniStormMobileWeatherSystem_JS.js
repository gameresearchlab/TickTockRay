//UniStorm Mobile Weather System JavaScript Version 2.1.4 @ Copyright
//Black Horizon Studios
	...
	var lightningEnabled : boolean = false;
	var useWindZone : boolean = false;	
...
function Awake () 
	...
	function GetAllComponents ()
	{
		...
		if (cloudDensity == 1)
		{
			partlyCloudyClouds2Component.gameObject.SetActive(false);
			lightClouds1aComponent.gameObject.SetActive(false);
			mostlyCloudyClouds2Component.gameObject.SetActive(false);

			if (partlyCloudyClouds1Component.material.color.a <= 0)
			{
				partlyCloudyClouds1Component.gameObject.SetActive(false);
			}
		}
	}
	...
	function Lightning () 
	{
			...
			if (fadeLightning == true)
			{
				...
				if (lightSourceComponent.intensity <= 0)
				{
					...
					lightningOdds = UnityEngine.Random.Range(0.5f, lightningMaxChance);
	...
	function FadeOutPrecipitation ()
	{
		...
		if (weatherForecaster == 11)
		{
			...
			if(heavyCloudsComponent.material.color.a <= 0.33f)
			{
		...
		if (cloudType == 2 && weatherForecaster == 7)
		{
			...
			if(weatherForecaster == 7)
			{
				...
				{
					partlyCloudyClouds1Component.enabled = false;
					lightClouds1Component.enabled = true;
					mostlyCloudyClouds1Component.enabled = false;
				}
		...
		if(weatherForecaster == 8)
		{
			...
			*/
			...
			if (minRainIntensity <= 0 && useWindZone)
			{
				minRainIntensity = 0;
			...
			if (minRainIntensity >= 1 && useWindZone)
			{
	...
	function FadeInPrecipitation()
	{
			...
			if (temperature >= 33 && temperatureType == 1  && weatherForecaster == 3 || temperatureType == 2 && temperature >= 1 && weatherForecaster == 3 || temperature >= 33 && temperatureType == 1  && weatherForecaster == 12 || temperatureType == 2 && temperature >= 1 && weatherForecaster == 12)
			{
				...
				if (weatherForecaster == 3 && minRainIntensity >= 1)
				{
					Lightning ();
				}
